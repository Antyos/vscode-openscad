...
let csStatusBarItem: vscode.StatusBarItem;
let docIsSCAD:vscode.DocumentSelector = {language: 'scad'};


export function activate(context: vscode.ExtensionContext) {
    const csCommandId = 'scad.cheatsheet';
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand(csCommandId, () => {
            ...
            const columnToShowIn = vscode.window.activeTextEditor
                ? (vscode.workspace.getConfiguration().get('openscad.cheatsheet.openToSide') 
                  ? vscode.ViewColumn.Beside : vscode.window.activeTextEditor.viewColumn)
                : undefined;
            ...
            else 
            {
                ...
                currentPanel = vscode.window.createWebviewPanel(
                    ...
                    columnToShowIn || vscode.ViewColumn.One,        // Editor column
                    {
                ...
                // Set HTML content
                currentPanel.webview.html = getRawCS(); //getWebviewContent();
    ...
    csStatusBarItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 0);
    csStatusBarItem.command = csCommandId;
    context.subscriptions.push(csStatusBarItem);

    // Register listener to make sure status bar item is up-to-date
...
// Updates the cheatsheet status bar item
// Cheatsheet button should only show up when the active file is of language type 'scad'
function updateStatusBarItem(): void {
    if (docIsSCAD && vscode.workspace.getConfiguration().get('openscad.cheatsheet.displayInStatusBar'))
    {
        csStatusBarItem.text = 'OpenSCAD CheatSheet';
        csStatusBarItem.show();
    }
    else
    {
        csStatusBarItem.hide();
    }
}

async function getCheatsheet() {
    var result = await WebRequest.get('https://www.openscad.org/cheatsheet/');
